...
def df(x):
    return -2 * math.cos(x) * math.sin(x) - 0.5 * math.exp(0.3 * x) * (1 + 0.3 * x)

# MÉTODO DE BISECCIÓN
def metodo_biseccion(a, b, tol, max_iter=100):
    print("MÉTODO DE LA BISECCIÓN")
    print("=" * 80)
    print(f"Función: f(x) = cos²(x) - 0.5x e^(0.3x) + 5")
...
def metodo_newton_raphson(x0, tol, max_iter=100):
    ...
    print("=" * 80)
    print(f"Función: f(x) = cos²(x) - 0.5x e^(0.3x) + 5")
    print(f"Derivada: f'(x) = -2cos(x)sin(x) - 0.5e^(0.3x)(1 + 0.3x)")
...
def graficar_funcion():
    ...
    plt.plot(x, y, 'b-', linewidth=2.5, label='f(x) = cos²(x) - 0.5x e^(0.3x) + 5')
    plt.axhline(y=0, color='k', linestyle='-', alpha=0.3, linewidth=1)
    plt.axvline(x=0, color='k', linestyle='-', alpha=0.3, linewidth=1)
    plt.xlim(-7, 5)
    plt.ylim(-5, 8)
    raices = []
    raiz1, _ = metodo_biseccion(3, 4, 1e-6)
    if raiz1 is not None:
        raices.append(raiz1)
        plt.plot(raiz1, 0, 'ro', markersize=10, label=f'Raíz ≈ {raiz1:.6f}',
                 markeredgecolor='black', markeredgewidth=2)
    plt.grid(True, alpha=0.3, linestyle='--')
    plt.xlabel('x', fontsize=12, fontweight='bold')
    plt.ylabel('f(x)', fontsize=12, fontweight='bold')
    plt.title('Gráfica de f(x) = cos²(x) - 0.5x e^(0.3x) + 5', fontsize=14, fontweight='bold')
...
# PROGRAMA PRINCIPAL
def ejercicio_4():
    print("EJERCICIO 4 - ABIGAIL MAMANI")
    print("=" * 80)
    print("Función: f(x) = cos²(x) - 0.5x e^(0.3x) + 5")
    print()
    tolerancia = 1e-6
