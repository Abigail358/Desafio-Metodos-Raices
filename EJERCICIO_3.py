...
def df(x):
    return 3 * x ** 2 - ((2 * x - x ** 2) * math.exp(-0.5 * x)) - 3
...
def metodo_biseccion(a, b, tol, max_iter=100):
    ...
    print("=" * 80)
    print(f"Función: f(x) = x³ - x²e^(-0.5x) - 3x + 1")
...
def metodo_newton_raphson(x0, tol, max_iter=100):
    ...
    print("=" * 80)
    print(f"Función: f(x) = x³ - x²e^(-0.5x) - 3x + 1")
    print(f"Derivada: f'(x) = 3x² - [(2x - x²)e^(-0.5x)] - 3")
...
def graficar_funcion():
    ...
    y = f_np(x)
    plt.plot(x, y, 'b-', linewidth=2.5, label='f(x) = x³ - x²e^(-0.5x) - 3x + 1')
    ...
    plt.ylim(-3, 3)
    raices = []
    raiz1, _ = metodo_biseccion(1, 2, 1e-6)
    if raiz1 is not None:
        raices.append(raiz1)
        plt.plot(raiz1, 0, 'ro', markersize=10, label=f'Raíz A ≈ {raiz1:.6f}',
                 markeredgecolor='black', markeredgewidth=2)

    raiz2, _ = metodo_biseccion(0, 1, 1e-6)
    if raiz2 is not None:
        raices.append(raiz2)
        plt.plot(raiz2, 0, 'go', markersize=10, label=f'Raíz B ≈ {raiz2:.2f}',
                 markeredgecolor='black', markeredgewidth=2)

    raiz3, _ = metodo_biseccion(-2, -1, 1e-6)
    if raiz2 is not None:
        raices.append(raiz3)
        plt.plot(raiz3, 0, 'ro', markersize=10, label=f'Raíz C ≈ {raiz3:.2f}',
                 markeredgecolor='black', markeredgewidth=2)

    plt.grid(True, alpha=0.3, linestyle='--')
    plt.xlabel('x', fontsize=12, fontweight='bold')
    plt.ylabel('f(x)', fontsize=12, fontweight='bold')
    plt.title('Gráfica de f(x) = x³ - x²e^(-0.5x) - 3x + 1', fontsize=14, fontweight='bold')
    ...
    print("=" * 50)
    for i, raiz in enumerate(raices):
        print(f"Raíz: x ≈ {raiz:.6f}, f(x) = {f(raiz):.2e}")
...
# PROGRAMA PRINCIPAL
def ejercicio_3():
    print("EJERCICIO 3 - ABIGAIL MAMANI")
    print("=" * 80)
    print("Función: f(x) = x³ - x²e^(-0.5x) - 3x + 1")
    print()
    ...
    print("\nGenerando gráfica...encontrando raices")
    graficar_funcion()
